...
def request_embedding(text: str):
    with requests.post("http://127.0.0.1:8000/embedding/", json={"query": text}) as response:
        body = response.json()
    return body["embedding"]

def request_cross_embedding(query: str, context: str):
    with requests.post("http://127.0.0.1:8000/cross_embedding/", json={"query": query, "context": context}) as response:
        body = response.json()

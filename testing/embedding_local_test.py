...
from job_seeker.embedding.sentence_transformer_embedding import request_embedding, request_cross_embedding
...
def bi_encoder_test():
    ...
    with open("resume.json") as f:
        resume = json.load(f)
    resume_embeddings = {key: [request_embedding(text) for text in texts] for key, texts in resume.items()}
        ...
        for key, embedding_list in resume_embeddings.items():
            ...
            for i, resume_embedding in enumerate(embedding_list):
                ...
                for j, job_embedding in enumerate(job_embeddings):
                    cosine_similarity = np.dot(resume_embedding, job_embedding) / (np.linalg.norm(resume_embedding) * np.linalg.norm(job_embedding))
